...
to go
  iterate  ; do one iteration of sine map
  update-plot
 ...
 set num-turtles-created num-turtles-created + 1
  ...
  [
    ...
    [
      ...
      set ycor ((R / 4) * sin (180 * x)) * max-pycor
    ]
  ]
  ask turtles [die]
end
...
This model demonstrates the behavior of the sine map, x{t+1} = R/4 (sin(pi * x{t}), where x{t} is the value of x at time step t, and x{t+1} is the value of x at the next time step. x is always between 0 and 1.   R is a control parameter that ranges from 0 to 4.  


## HOW TO USE IT

Use the sliders to set R and x{0}.  Click on "setup" to draw the axes and the parabola representing the function y = R x (1 - x).  Click on "go" to do successive iterations of the logistic map.  
...
If you use this model, please cite it as: "Sine Map" model, Complexity Explorer project, http://complexityexplorer.org
