...
to setup
  ...
  ;; bar is "crowded"
  ask patch (0.80 * max-pxcor) (0.5 * max-pycor) [
    set crowded-patch self
    set plabel-color red
    ask crowded-patch [ set plabel "El Farol Bar" ]
  ]
...
to go
  ...
  if attendance > overcrowding-threshold [
    set total-crowded-days total-crowded-days + 1
  ]
...
to update-strategies
  ...
  foreach strategies [ ?1 ->
    ...
    let week 1
    repeat memory-size [
      set prediction predict-attendance ?1 sublist history week (week + memory-size)
...
to-report predict-attendance [strategy subhistory]
  ...
  report first strategy + sum (map [ [?1 ?2] -> ?1 * ?2 ] butfirst strategy subhistory)
end
...
to-report percent-crowded-days
  ifelse (ticks > 0) [report 100 * (total-crowded-days / ticks)] [report 0]
end
...
261
379
Number of crowded days as a percent of all days so far
11
0.0
1
...
This version of the model was slightly modified by Melanie Mitchell. 
