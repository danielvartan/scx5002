turtles-own [
  ...
  cardinal-direction ;; cardinal direction of heading (N, S, E, or W)
]

globals [max-align-turn max-cohere-turn max-separate-turn N-fraction S-fraction W-fraction E-fraction]
...
to setup
  ...
  ask turtles [set cardinal-direction coarse-grain-heading heading set label cardinal-direction]
  ask turtles [set-direction-colors]
  compute-direction-fractions
  reset-ticks
end

to-report coarse-grain-heading [heading-value]
  ; "north" (316 to 45)
  ; "east" (46 to 135)
  ; "south" (136 to 225)
  ; "west" (226 to 315)

  ifelse ((heading-value >= 316) or (heading-value <= 45))
    [report "N"]
    [ifelse ((heading-value >= 46) and (heading-value <= 135))
        [report "E"]
        [ifelse ((heading-value >= 136) and (heading-value <= 225))
            [report "S"]
            [report "W"]]]
end

to set-direction-colors ; turtle procedure
  if (cardinal-direction = "N") [set color blue]
  if (cardinal-direction = "S") [set color red]
  if (cardinal-direction = "E") [set color orange]
  if (cardinal-direction = "W") [set color green]
end

to compute-direction-fractions
  let directions [cardinal-direction] of turtles
  set N-fraction (length filter [ ?1 -> ?1 = "N" ] directions) / population
  set S-fraction (length filter [ ?1 -> ?1 = "S" ] directions) / population
  set E-fraction (length filter [ ?1 -> ?1 = "E" ] directions) / population
  set W-fraction (length filter [ ?1 -> ?1 = "W" ] directions) / population
end

to-report heading-entropy
  ; returns entropy of list of headings, where headings are binned into
  ; cardinal directions (north, south, east, west)
  let N-term (ifelse-value (N-fraction = 0) [0] [N-fraction * log N-fraction 2])
  let S-term (ifelse-value (S-fraction = 0) [0] [S-fraction * log S-fraction 2])
  let E-term (ifelse-value (E-fraction = 0) [0] [E-fraction * log E-fraction 2])
  let W-term (ifelse-value (W-fraction = 0) [0] [W-fraction * log W-fraction 2])
   report (-1 * (N-term + S-term + E-term + W-term))
end
  ...
  if any? flockmates
    [ find-nearest-neighbor
      ifelse distance nearest-neighbor < minimum-separation
        [ separate ]
        [ align   ; alternative:  align-with-nearest-neighbor
          cohere ] ]
end
...
to align-with-nearest-neighbor  ;; turtle procedure
  turn-towards [heading] of nearest-neighbor max-align-turn
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot heading-entropy"
...
<experiments>
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>heading-entropy = 0</exitCondition>
    <metric>ticks</metric>
