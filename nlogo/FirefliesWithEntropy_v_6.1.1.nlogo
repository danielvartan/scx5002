...
globals [entropy-clock completely-synchronized]
...
to go
  ...
  ; Stop when Shannon Entropy is 0 for cycle-length ticks
  ifelse (shannon-entropy = 0)
     [set entropy-clock entropy-clock + 1]
     [set entropy-clock 0]
  if (entropy-clock = cycle-length) [set completely-synchronized true stop]
  tick
end
...
to-report shannon-entropy
  let flashing-fraction (count turtles with [color = yellow]) / number
  let non-flashing-fraction (1 - flashing-fraction)
  let H  -1 * (ifelse-value ((flashing-fraction = 0) or (non-flashing-fraction = 0)) [0]
    [(flashing-fraction * log flashing-fraction  2) + (non-flashing-fraction * log non-flashing-fraction  2)])
  report H
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot shannon-entropy"
...
565
Fraction Flashing
(count turtles with [color = yellow]) / number
2
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="fireflies-experiment" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>completely-synchronized = true</exitCondition>
    <metric>ticks</metric>
