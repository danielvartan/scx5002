...
to setup ;; construct and rewire a ring graph
  ...
  create-ordered-nodes node-count
  ask nodes [setup-node fd 14]
...
to show-path-lengths show-path-length-to pick-node end
to show-path-length-to [a-node]
...
to show-degrees
  ask nodes [set label-color 0 set label count my-links]
  clear-legend ask nodes [set color 8]
end
...
to show-clusterings ;; to two decimal places
...
to show-grey-legend [white-value black-value legend-label]
  ...
  foreach n-values 10 [ ?1 -> ?1 ] [ ?1 -> ask patch (15 - ?1) -15 [set pcolor ?1] ]
...
to-report average-distance-to [a-node]
  show-path-length-to a-node
  report mean filter is-number? [label] of nodes
end

to-report average-path-length
  let average mean filter [ ?1 -> ?1 > 0 ] map average-distance-to sort nodes
...
The _clustering coefficient C(n)_ of a node _n_ is a fraction between 0 and 1: the number of links which actually do exist between these neighbors, out of the total number of such links which could potentially exist. You can see the individual _C(n)_ as numbers or as shades of grey (darker is higher). The global clustering coefficient (the average _C(i)_ over all nodes _i_) is shown in the lower right, in the **clustering-coefficient** box.
...
This model is part of the Network series of the Complexity Explorer project.
...
If you use this model, please cite it as: "SmallWorldNetworks" model, Complexity Explorer project, http://complexityexplorer.org
