...
to make-turtles
  ...
  crt num-unforgiving [ set strategy "unforgiving" set color magenta ]
  crt num-unknown [set strategy "unknown" set color black]
end
...
to go
  ...
  do-scoring
  calculate-highest-scoring-strategy
  if ticks = num-timesteps [stop]
  tick
end

to calculate-highest-scoring-strategy
  ifelse num-random-games > 0
     [set random-average (random-score / num-random-games)]
     [set random-average 0]
  ifelse num-tit-for-tat-games > 0
     [set tit-for-tat-average (tit-for-tat-score / num-tit-for-tat-games)]
     [set tit-for-tat-average 0]
  ifelse num-cooperate-games > 0
     [set cooperate-average (cooperate-score / num-cooperate-games)]
     [set cooperate-average 0]
  ifelse num-unforgiving-games > 0
     [set unforgiving-average (unforgiving-score / num-unforgiving-games)]
     [set unforgiving-average 0]
  ifelse num-defect-games > 0
     [set defect-average (defect-score / num-defect-games)]
     [set defect-average 0]
  ifelse num-unknown-games > 0
     [set unknown-average (unknown-score / num-unknown-games)]
     [set unknown-average 0]

  let averages-list (list random-average tit-for-tat-average cooperate-average unforgiving-average defect-average unknown-average)
  let max-average-score max averages-list
  let max-position position max-average-score averages-list
  set highest-scoring-strategy item max-position (list "random" "tit-for-tat" "cooperate" "unforgiving" "defect"  "unknown")
end
...
;to-report total-average-payoff
;  let sum-average-payoff 0
;  if (num-random-games > 0) [set sum-average-payoff sum-average-payoff + random-score / num-random-games ]
;  if (num-cooperate-games > 0) [set sum-average-payoff sum-average-payoff + cooperate-score / num-cooperate-games ]
;  if (num-defect-games > 0) [set sum-average-payoff sum-average-payoff + defect-score / num-defect-games ]
;  if (num-tit-for-tat-games > 0) [set sum-average-payoff sum-average-payoff + tit-for-tat-score / num-tit-for-tat-games]
;  if (num-unforgiving-games > 0) [set sum-average-payoff sum-average-payoff + unforgiving-score / num-unforgiving-games]
;  if (num-unknown-games > 0) [set sum-average-payoff sum-average-payoff + unknown-score / num-unknown-games]
;  report sum-average-payoff
;end
...
"unforgiving" 1.0 0 -1184463 true "" "if num-unforgiving-games > 0 [ plot unforgiving-score / (num-unforgiving-games) ]"
"unknown" 1.0 0 -16777216 true "" "if num-unknown-games > 0 [ plot unknown-score / (num-unknown-games) ]"
...
(sum [score] of turtles) / (sum (list num-random-games num-cooperate-games num-defect-games\nnum-tit-for-tat-games num-unforgiving-games num-unknown-games))
2
1
11
...
sum (list num-random-games num-cooperate-games num-defect-games\nnum-tit-for-tat-games num-unforgiving-games num-unknown-games)
17
1
11
